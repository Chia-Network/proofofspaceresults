...
#include <chia/calculate_bucket.hpp>
#include <chia/sort_on_disk.hpp>
...
// Number of buckets to use for sort_on_disk.
...
struct phase3_results {
    // Pointers to each table start byte in the plot file
    std::vector<uint64_t> plot_table_begin_pointers;
...
public:
    ...
    // the process.
    void create_plot_disk(const std::string &filename, uint8_t k, uint8_t *memo, uint32_t memo_len, uint8_t *id,
                          uint32_t id_len) {
        std::cout << std::endl << "Starting plotting progress into file " << filename << "." << std::endl;
        std::cout << "Memo: " << utilities::hex_str(memo, memo_len) << std::endl;
        std::cout << "ID: " << utilities::hex_str(id, id_len) << std::endl;
        std::cout << "Plot size is: " << static_cast<int>(k) << std::endl;

        // These variables are used in the write_park_to_file method. They are preallocatted here
        // to save time.
        first_line_point_bytes = new uint8_t[calculate_line_point_size(k)];
        park_stubs_bytes = new uint8_t[calculate_stubs_size(k)];
        park_deltas_bytes = new uint8_t[calculate_max_deltas_size(k, 1)];
        ...
        timer p1;
        timer all_phases;
        std::vector<uint64_t> results = write_plot_file(plot_filename, k, id, memo, memo_len);
        p1.print_elapsed("Time for phase 1 =");
        ...
        p2.print_elapsed("Time for phase 2 =");

        std::cout << std::endl << "Starting phase 3/4: Compression..." << std::endl;
        timer p3;
        phase3_results res = compress_tables(k, results, filename, plot_filename, id, memo, memo_len);
        p3.print_elapsed("Time for phase 3 =");

        std::cout << std::endl << "Starting phase 4/4: write Checkpoint tables..." << std::endl;
        timer p4;
        write_c_tables(k, k + 1, filename, plot_filename, res);
        p4.print_elapsed("Time for phase 4 =");
        ...
        std::cout << "Final File size: "
                  << static_cast<double>(res.final_table_begin_pointers[11]) / (1024 * 1024 * 1024) << " GB"
                  << std::endl;
        all_phases.print_elapsed("Total time =");
    ...
    static uint32_t get_max_entry_size(uint8_t k, uint8_t table_index, bool phase_1_size) {
        switch (table_index) {
            case 1:
                // Represents f1, x
                return utilities::byte_align(k + kExtraBits + k) / 8;
            case 2:
            case 3:
            case 4:
            case 5:
            case 6:
                if (phase_1_size)
                    ...
                    return utilities::byte_align(k + kExtraBits + (k + 1) + kOffsetSize +
                    ...
                    return utilities::byte_align(std::max(static_cast<uint32_t>(k + 1 + (k + 1) + kOffsetSize),
                                                          static_cast<uint32_t>(2 * k + k + 1))) /
                           8;
            case 7:
            default:
                // Represents line_point, f7
                return utilities::byte_align(3 * k) / 8;
        }
    }
    ...
    static uint32_t calculate_c3_size(uint8_t k) {
        if (k < 20) {
            return floor(utilities::byte_align(8 * kCheckpoint1Interval) / 8);
        } else {
            // TODO(alex): tighten this bound, based on formula
            return utilities::byte_align(kC3BitsPerEntry * kCheckpoint1Interval) / 8;
        }
    }

    static uint32_t calculate_line_point_size(uint8_t k) {
        return utilities::byte_align(2 * k) / 8;
    }

    // This is the full size of the deltas section in a park. However, it will not be fully filled
    static uint32_t calculate_max_deltas_size(uint8_t k, uint8_t table_index) {
        if (table_index == 1) {
            return utilities::byte_align((kEntriesPerPark - 1) * kMaxAverageDeltaTable1) / 8;
        }
        return utilities::byte_align((kEntriesPerPark - 1) * kMaxAverageDelta) / 8;
    }

    static uint32_t calculate_stubs_size(uint k) {
        return utilities::byte_align((kEntriesPerPark - 1) * (k - kStubMinusBits)) / 8;
    }

    static uint32_t calculate_park_size(uint8_t k, uint8_t table_index) {
        return calculate_line_point_size(k) + calculate_stubs_size(k) + calculate_max_deltas_size(k, table_index);
    }
...
private:
    ...
    uint32_t write_header(std::ofstream &plot_file, uint8_t k, uint8_t *id, uint8_t *memo, uint32_t memo_len) {
    ...
    std::vector<uint64_t> write_plot_file(const std::string &plot_filename, uint8_t k, uint8_t *id, uint8_t *memo,
                                          uint8_t memo_len) {
        ...
        if (!plot_file.is_open()) {
            throw std::invalid_argument("File not opened correct");
        }
        uint32_t header_size = write_header(plot_file, k, id, memo, memo_len);

        std::cout << "Computing table 1" << std::endl;
        timer f1_start_time;
        f1_calculator f1(k, id);

        uint32_t entry_size_bytes = get_max_entry_size(k, 1, true);
        ...
        // max(X) fits into uint64_t
        uint64_t x = 0;
        uint64_t number_of_evaluations = 1 << kBatchSizes; //uint64_t number_of_evaluations = 2 << (kBatchSizes - 1);

        size_t count_first = 0;
        size_t count_second = 0;

//        for ( const auto& kv: f1.calculate_buckets(x, max_value + 1)) {
//            bits xy_bits = kv + bits(x, k);
//            xy_bits.to_bytes(buf);
//
//            // We write the x, y pair
//            plot_file.write(reinterpret_cast<char *>(buf), entry_size_bytes);
//
//            bucket_sizes[sort_on_disk_utils::extract_num(buf, entry_size_bytes, 0, bucket_log)] += 1;
//        }
        for (uint64_t lp = 0; lp <= (((uint64_t)1) << (k - kBatchSizes)); lp++) {
            // For each pair x, y in the batch
            for ( const uint128_t& kv: f1.calculate_buckets(x, number_of_evaluations)) {
                // TODO(mariano): fix inefficient memory alloc here
                uint128_t xy = (kv << k) + x;
                //bits xy_bits = kv + bits(x, k);
                //xy_bits.to_bytes(buf); //(std::get<0>(kv) + std::get<1>(kv)).to_bytes(buf);

                // We write the x, y pair
                uint8_t xy_bite_size = kExtraBits + 2 * k;
                uint8_t buf_bite_size = entry_size_bytes * 8;
                xy = xy << (buf_bite_size - xy_bite_size);
                for (size_t i = 0; i < entry_size_bytes; ++i) {
                    buf[entry_size_bytes - i - 1] = xy & 0xffU;
                    xy = xy >> 8;
                }
                plot_file.write(reinterpret_cast<char *>(buf), entry_size_bytes);
        ...
        utilities::write_zeroes_stack(plot_file, entry_size_bytes);
        plot_file.close();
        f1_start_time.print_elapsed("F1 complete, Time = ");
        ...
        plot_entry *bucket_L = new plot_entry[kBucketMaxSize];
        plot_entry *bucket_R = new plot_entry[kBucketMaxSize];
        ...
        for (uint8_t table_index = 1; table_index < 7; table_index++) {
            ...
            uint32_t entry_size_bytes = get_max_entry_size(k, table_index, true);
            right_entry_size_bytes = get_max_entry_size(k, table_index + 1, true);

            uint64_t begin_byte_next = begin_byte + (entry_size_bytes * (total_table_entries + 1));

            std::cout << "Computing table " << int {table_index + 1} << " at position 0x" << std::hex << begin_byte_next
                      << std::dec << std::endl;
            ...
            timer sort_timer;
            file_disk d(plot_filename);
            sorting::sort_on_disk(d, begin_byte, begin_byte_next, entry_size_bytes, 0, bucket_sizes, memory,
                                  kMemorySize);
            d.close();
            sort_timer.print_elapsed("\tSort time:");
            ...
            fx_calculator f(k, table_index + 1, id);
            ...
            uint64_t size_bucket_L = 0;
            uint64_t size_bucket_R = 0;
            ...
            while (!end_of_table) {
                ...
                if (table_index == 1) {
                    ...
                    left_entry.y = utilities::slice_int64_from_bytes(left_buf, entry_size_bytes, 0, k + kExtraBits);
                    left_entry.left_metadata =
                        utilities::slice_int128_from_bytes(left_buf, entry_size_bytes, k + kExtraBits, metadata_size);
                } else {
                    // For tables 2-6, we we also have pos and offset, but we don't use it here.
                    left_entry.y = utilities::slice_int64_from_bytes(left_buf, entry_size_bytes, 0, k + kExtraBits);
                    if (metadata_size <= 128) {
                        left_entry.left_metadata = utilities::slice_int128_from_bytes(
                            left_buf, entry_size_bytes, k + kExtraBits + pos_size + kOffsetSize, metadata_size);
                    } else {
                        // Large metadatas that don't fit into 128 bits. (k > 32).
                        left_entry.left_metadata = utilities::slice_int128_from_bytes(
                            left_buf, entry_size_bytes, k + kExtraBits + pos_size + kOffsetSize, 128);
                        left_entry.right_metadata = utilities::slice_int128_from_bytes(
                            left_buf, entry_size_bytes, k + kExtraBits + pos_size + kOffsetSize + 128,
                            metadata_size - 128);
                    }
                }
                ...
                if (y_bucket == bucket) {
                    bucket_L[size_bucket_L] = left_entry;
                    ++size_bucket_L;
                } else if (y_bucket == bucket + 1) {
                    bucket_R[size_bucket_R] = left_entry;
                    ++size_bucket_R;
                } else {
                    ...
                    if (size_bucket_L && size_bucket_R) {
                        // Compute all matches between the two buckets, and return indeces into each bucket
                        std::vector<std::pair<uint16_t, uint16_t>> match_indexes = f.find_matches(bucket_L, size_bucket_L, bucket_R, size_bucket_R);
                        for (auto &indeces : match_indexes) {
                            plot_entry &L_entry = bucket_L[std::get<0>(indeces)];
                            plot_entry &R_entry = bucket_R[std::get<1>(indeces)];

                            std::pair<zerg_bits, zerg_bits> zerg_f_output;

                            // Computes the output pair (fx, new_metadata)
                            // Чтобы определить границы можно считать исходную имплементацию эталонной
                            // здесь в функции аес разбивается максимум на 4 блок по 128, это значит
                            // что metadata_size <= 128, из чего 4k <= 128 => max k = 32;
                            if (metadata_size <= 128) {
                                zerg_f_output = f.zerg_calculate_bucket128(zerg_bits(L_entry.y, k + kExtraBits),
                                                                        zerg_bits(L_entry.left_metadata, metadata_size),
                                                                        zerg_bits(R_entry.left_metadata, metadata_size));
                            } else {
                                // Metadata does not fit into 128 bits
                                zerg_f_output = f.zerg_calculate_bucket256(zerg_bits(L_entry.y, k + kExtraBits),
                                                                        zerg_bits(L_entry.left_metadata, 128) +
                                                                            zerg_bits(L_entry.right_metadata, metadata_size - 128),
                                                                        zerg_bits(R_entry.left_metadata, 128) +
                                                                            zerg_bits(R_entry.right_metadata, metadata_size - 128));
                            }
                            // На данном этапе zerg_bits точно хватает
                            // fx/y, which will be used for sorting and matching

                            zerg_bits &new_entry = std::get<0>(zerg_f_output);
                            uint64_t  new_entry_len = new_entry.len;
                            ++matches;
                            ++total_table_entries;
                            ...
                            new_entry_len += pos_size;
                            // Offset for matching entry
                            if (R_entry.pos - L_entry.pos > 2000) {
                                std::cout << "Offset: " << R_entry.pos - L_entry.pos << std::endl;
                            }
                            new_entry_len += kOffsetSize;
                            // New metadata which will be used to compute the next f
                            new_entry_len += std::get<1>(zerg_f_output).len;
                            // Fill with 0s if entry is not long enough
                            int64_t zero_bits_count = right_entry_size_bytes * 8 - new_entry_len;
                            new_entry_len += zero_bits_count;
                            uint64_t shift = utilities::byte_align(new_entry_len) - new_entry_len;
                            zero_bits_count += shift;
                            // Writes the new entry into the right table

                            //Надо как-то насчитать буффер и вывести его
                            uint64_t buff_pos = right_entry_size_bytes - 1 - zero_bits_count / 8;
                            zerg_bits work_buf(0, zero_bits_count % 8);
                            work_buf = std::get<1>(zerg_f_output) + work_buf;
                            while (work_buf.len >= 8) {
                                right_buf[buff_pos] = work_buf.value & 0xffU;
                                work_buf.value >>= 8U;
                                work_buf.len -= 8;
                                buff_pos--;
                            }
                            work_buf = zerg_bits(L_entry.pos, pos_size) + zerg_bits(R_entry.pos - L_entry.pos, kOffsetSize) + work_buf;
                            while (work_buf.len >= 8) {
                                right_buf[buff_pos] = work_buf.value & 0xffU;
                                work_buf.value >>= 8U;
                                work_buf.len -= 8;
                            ...
                            right_bucket_sizes[sort_on_disk_utils::extract_num(right_buf, right_entry_size_bytes, 0,
                    ...
                    if (y_bucket == bucket + 2) {
                        ...
                        std::swap(bucket_L, bucket_R); //bucket_L = bucket_R;
                        //bucket_R = std::vector<plot_entry>();
                        bucket_R[0] = left_entry;
                        size_bucket_L = size_bucket_R;
                        size_bucket_R = 1;
                        ++bucket;
                    } else {
                        // We saw a bucket that >2 more than the current, so we just set L = [entry], and R = []
                        bucket = y_bucket;
                        //bucket_L = std::vector<plot_entry>();
                        //bucket_L.emplace_back(left_entry);
                        bucket_L[0] = left_entry;
                        size_bucket_L = 1;
                        size_bucket_R = 0; //bucket_R = std::vector<plot_entry>();
                    }
                }
            ...
            std::cout << "\tTotal matches: " << matches << ". Per bucket: " << (matches / num_buckets) << std::endl;

            // Writes the 0 entry (EOT)
            memset(right_buf, 0, right_entry_size_bytes);
            bits(0, right_entry_size_bytes * 8).to_bytes(right_buf);
            right_writer.write(reinterpret_cast<char *>(right_buf), right_entry_size_bytes);
            ...
            bits(begin_byte_next, 8 * 8).to_bytes(pointer_buf);
            ...
            computation_pass_timer.print_elapsed("\tComputation pass time:");
            table_timer.print_elapsed("Forward propagation table time:");
        }
        delete [] bucket_L;
        delete [] bucket_R;
        // Pointer to the end of the last table + 1, used for spare space for disk sorting
        plot_table_begin_pointers[8] =
            plot_table_begin_pointers[7] + (right_entry_size_bytes * (total_table_entries + 1));
        delete[] memory;
        std::cout << "Final plot table begin pointers: " << std::endl;
        for (uint8_t i = 1; i <= 8; i++) {
            std::cout << "\tTable " << int {i} << " 0x" << std::hex << plot_table_begin_pointers[i] << std::dec
                      << std::endl;
        }
    ...
    void backpropagate(const std::string &filename, const std::string &plot_filename, uint8_t k, uint8_t *id,
                       uint8_t *memo, uint32_t memo_len, const std::vector<uint64_t> &results) {
        ...
        for (uint8_t table_index = 7; table_index > 1; --table_index) {
            ...
            uint16_t left_entry_size_bytes = get_max_entry_size(k, table_index - 1, true);

            // The entry that we are writing (no metadata)
            uint16_t new_left_entry_size_bytes = get_max_entry_size(k, table_index - 1, false);

            // The right entries which we read and write (the already have no metadata, since they have
            // been pruned in previous iteration)
            uint16_t right_entry_size_bytes = get_max_entry_size(k, table_index, false);
            ...
            if (table_index != 7) {
                ...
                timer sort_timer;
                file_disk d(plot_filename);
                sorting::sort_on_disk(d, plot_table_begin_pointers[table_index], spare_pointer, right_entry_size_bytes,
            ...
            static_assert(kCachedPositionsSize % 2 == 0, "");
            ...
            for (unsigned short &old_counter : old_counters) {
                old_counter = 0;
            }
                ...
                if (!end_of_right_table || current_pos <= greatest_pos) {
                    ...
                    while (!end_of_right_table) {
                        if (should_read_entry) {
                            ...
                            right_reader.read(reinterpret_cast<char *>(right_entry_buf), right_entry_size_bytes);
                            if (table_index == 7) {
                                // This is actually y for table 7
                                entry_sort_key =
                                    utilities::slice_int64_from_bytes(right_entry_buf, right_entry_size_bytes, 0, k);
                                entry_pos = utilities::slice_int64_from_bytes(right_entry_buf, right_entry_size_bytes,
                                                                              k, pos_size);
                                entry_offset = utilities::slice_int64_from_bytes(
                                    right_entry_buf, right_entry_size_bytes, k + pos_size, kOffsetSize);
                            } else {
                                entry_pos = utilities::slice_int64_from_bytes(right_entry_buf, right_entry_size_bytes,
                                                                              0, pos_size);
                                entry_offset = utilities::slice_int64_from_bytes(
                                    right_entry_buf, right_entry_size_bytes, pos_size, kOffsetSize);
                                entry_sort_key = utilities::slice_int64_from_bytes(
                                    right_entry_buf, right_entry_size_bytes, pos_size + kOffsetSize, k + 1);
                            }
                    ...
                    // Reads a left entry
                    left_reader.read(reinterpret_cast<char *>(left_entry_buf), left_entry_size_bytes);

                    // If this left entry is used, we rewrite it. If it's not used, we ignore it.
                    if (used_positions[current_pos % kCachedPositionsSize]) {
                        uint64_t entry_y =
                            utilities::slice_int64_from_bytes(left_entry_buf, left_entry_size_bytes, 0, k + kExtraBits);
                        uint64_t entry_metadata;

                        if (table_index > 2) {
                            // For tables 2-6, the entry is: f, pos, offset metadata
                            entry_pos = utilities::slice_int64_from_bytes(left_entry_buf, left_entry_size_bytes,
                                                                          k + kExtraBits, pos_size);
                            entry_offset = utilities::slice_int64_from_bytes(left_entry_buf, left_entry_size_bytes,
                                                                             k + kExtraBits + pos_size, kOffsetSize);
                        } else {
                            // For table1, the entry is: f, metadata
                            entry_metadata = utilities::slice_int128_from_bytes(left_entry_buf, left_entry_size_bytes,
                        ...
                        if (table_index > 2) {
                            ...
                            if (utilities::byte_align(new_left_entry.size()) < new_left_entry_size_bytes * 8) {
                            ...
                            // std::cout << "Writing X:" << entry_metadata.get_value() << std::endl;
                        }
                        new_left_entry.to_bytes(new_left_entry_buf);
                        left_writer.write(reinterpret_cast<char *>(new_left_entry_buf), new_left_entry_size_bytes);

                        new_bucket_sizes_pos[sort_on_disk_utils::extract_num(
                            new_left_entry_buf, new_left_entry_size_bytes, 0, floor(log2(kNumSortBuckets)))] += 1;
                ...
                if (write_pointer_pos >= 0 && used_positions[write_pointer_pos % kCachedPositionsSize]) {
                    ...
                    for (uint32_t counter = 0; counter < old_counters[write_pointer_pos % kReadMinusWrite]; counter++) {
                        ...
                        uint64_t new_offset_pos =
                            new_positions[old_offsets[write_pointer_pos % kReadMinusWrite][counter] %
                        ...
                        new_right_entry.append_value(new_offset_pos - new_pos, kOffsetSize);
                        if (utilities::byte_align(new_right_entry.size()) < right_entry_size_bytes * 8) {
                            memset(right_entry_buf, 0, right_entry_size_bytes);
                        }
                        new_right_entry.to_bytes(right_entry_buf);
            ...
            computation_pass_timer.print_elapsed("\tComputation pass time:");
            table_timer.print_elapsed("Total backpropagation time::");

            bits(0, right_entry_size_bytes * 8).to_bytes(right_entry_buf);
            right_writer.write(reinterpret_cast<char *>(right_entry_buf), right_entry_size_bytes);
            bits(0, new_left_entry_size_bytes * 8).to_bytes(new_left_entry_buf);
            left_writer.write(reinterpret_cast<char *>(new_left_entry_buf), new_left_entry_size_bytes);
    ...
    void write_park_to_file(std::ofstream &writer, uint64_t table_start, uint64_t park_index, uint32_t park_size_bytes,
        ...
        memset(first_line_point_bytes, 0, calculate_line_point_size(k));
        first_line_point_bits.to_bytes(first_line_point_bytes);
        writer.write((const char *)first_line_point_bytes, calculate_line_point_size(k));

        // We use park_bits insted of bits since it allows storing more data
        park_bits park_stubs_bits;
        for (uint64_t stub : park_stubs) {
            park_stubs_bits.append_value(stub, (k - kStubMinusBits));
        }
        uint32_t stubs_size = calculate_stubs_size(k);
        memset(park_stubs_bytes, 0, stubs_size);
        park_stubs_bits.to_bytes(park_stubs_bytes);
        ...
        park_bits deltas_bits = encoding::ans_encode_deltas(park_deltas, R);
        deltas_bits.to_bytes(park_deltas_bytes);

        uint16_t encoded_size = deltas_bits.size() / 8;
        assert((uint32_t)(encoded_size + 2) < calculate_max_deltas_size(k, table_index));
    ...
    phase3_results compress_tables(uint8_t k, const std::vector<uint64_t> &plot_table_begin_pointers,
                                   const std::string &filename, const std::string &plot_filename, uint8_t *id,
        ...
        uint32_t header_size = write_header(header_writer, k, id, memo, memo_len);
        ...
        bits(final_table_begin_pointers[1], 8 * 8).to_bytes(table_1_pointer_bytes);
        ...
        for (uint8_t table_index = 1; table_index < 7; table_index++) {
            ...
            uint32_t park_size_bytes = calculate_park_size(k, table_index);
            ...
            uint32_t left_entry_size_bytes = get_max_entry_size(k, table_index, false);
            right_entry_size_bytes = get_max_entry_size(k, table_index + 1, false);
                ...
                if (end_of_right_table || current_pos <= greatest_pos) {
                    while (!end_of_right_table) {
                        if (should_read_entry) {
                            ...
                            right_reader.read(reinterpret_cast<char *>(right_entry_buf), right_entry_size_bytes);
                            entry_sort_key = utilities::slice_int64_from_bytes(right_entry_buf, right_entry_size_bytes,
                                                                               0, right_sort_key_size);
                            entry_pos = utilities::slice_int64_from_bytes(right_entry_buf, right_entry_size_bytes,
                                                                          right_sort_key_size, pos_size);
                            entry_offset = utilities::slice_int64_from_bytes(
                                right_entry_buf, right_entry_size_bytes, right_sort_key_size + pos_size, kOffsetSize);
                        } else if (cached_entry_pos == current_pos) {
                            entry_sort_key = cached_entry_sort_key;
                            entry_pos = cached_entry_pos;
                            entry_offset = cached_entry_offset;
                    ...
                    if (table_index == 1) {
                        ...
                        left_new_pos[current_pos % kCachedPositionsSize] = utilities::slice_int64_from_bytes(
                            left_entry_disk_buf, left_entry_size_bytes, left_y_size, k);
                    } else {
                        // k+1 bits in case it overflows
                        left_new_pos[current_pos % kCachedPositionsSize] = utilities::slice_int64_from_bytes(
                            left_entry_disk_buf, left_entry_size_bytes, k + 1, pos_size);
                    }
                }
                ...
                if (current_pos + 1 >= kReadMinusWrite) {
                    ...
                    for (uint32_t counter = 0; counter < old_counters[write_pointer_pos % kReadMinusWrite]; counter++) {
                        uint64_t left_new_pos_2 =
                            left_new_pos[old_offsets[write_pointer_pos % kReadMinusWrite][counter] %
                                         kCachedPositionsSize];

                        // A line point is an encoding of two k bit values into one 2k bit value.
                        uint128_t line_point = encoding::square_to_line_point(left_new_pos_1, left_new_pos_2);
                        ...
                        to_write.to_bytes(right_entry_buf);
                        right_writer.write((const char *)right_entry_buf, right_entry_size_bytes);
                        bucket_sizes[sort_on_disk_utils::extract_num(right_entry_buf, right_entry_size_bytes, 0,
                                                                     floor(log2(kNumSortBuckets)))] += 1;
                    }
                }
                current_pos += 1;
            ...
            computation_pass_1_timer.print_elapsed("\tFirst computation pass time:");
            timer sort_timer;
            std::cout << "\tsorting table " << int {table_index + 1} << std::endl;

            file_disk d = file_disk(plot_filename);
            sorting::sort_on_disk(d, plot_table_begin_pointers[table_index + 1], spare_pointer, right_entry_size_bytes,
            ...
            for (uint64_t index = 0; index < total_r_entries; index++) {
                right_reader_2.read(reinterpret_cast<char *>(right_entry_buf), right_entry_size_bytes);
                // Right entry is read as (line_point, sort_key)
                uint128_t line_point =
                    utilities::slice_int128_from_bytes(right_entry_buf, right_entry_size_bytes, 0, 2 * k);
                uint64_t sort_key = utilities::slice_int64_from_bytes(right_entry_buf, right_entry_size_bytes, 2 * k,
                                                                      right_sort_key_size);
                ...
                new_bucket_sizes[sort_on_disk_utils::extract_num(right_entry_buf, right_entry_size_bytes, 0,
                                                                 floor(log2(kNumSortBuckets)))] += 1;
                // Every EPP entries, writes a park
                if (index % kEntriesPerPark == 0) {
                    if (index != 0) {
                        write_park_to_file(final_table_writer, final_table_begin_pointers[table_index], park_index,
                                           park_size_bytes, checkpoint_line_point, park_deltas, park_stubs, k,
                                           table_index);
                        park_index += 1;
            ...
            if (!park_deltas.empty()) {
                // Since we don't have a perfect multiple of EPP entries, this writes the last ones
                write_park_to_file(final_table_writer, final_table_begin_pointers[table_index], park_index,
                                   park_size_bytes, checkpoint_line_point, park_deltas, park_stubs, k, table_index);
                final_entries_written += (park_stubs.size() + 1);
            }
            ...
            bits(final_table_begin_pointers[table_index + 1], 8 * 8).to_bytes(table_pointer_bytes);
            final_table_writer.write(reinterpret_cast<char *>(table_pointer_bytes), 8);

            final_table_writer.close();

            computation_pass_2_timer.print_elapsed("\tSecond computation pass time:");
            timer sort_timer_2;
            std::cout << "\tRe-sorting table " << int {table_index + 1} << std::endl;
            file_disk d_2 = file_disk(plot_filename);
            ...
            sorting::sort_on_disk(d_2, plot_table_begin_pointers[table_index + 1], spare_pointer,
                                  right_entry_size_bytes, 0, new_bucket_sizes, memory, kMemorySize);
            d_2.close();
            sort_timer_2.print_elapsed("\tSort time:");

            delete[] right_entry_buf;
            delete[] left_entry_disk_buf;

            table_timer.print_elapsed("Total compress table time:");
        }

        delete[] memory;

        // These results will be used to write table P7 and the checkpoint tables in phase 4.
        return phase3_results {plot_table_begin_pointers, final_table_begin_pointers, final_entries_written,
    ...
    void write_c_tables(uint8_t k, uint8_t pos_size, const std::string &filename, const std::string &plot_filename,
                        phase3_results &res) {
        ...
        uint32_t P7_park_size = utilities::byte_align((k + 1) * kEntriesPerPark) / 8;
        ...
        uint64_t total_C1_entries = ceil(res.final_entries_written / static_cast<double>(kCheckpoint1Interval));
        uint64_t begin_byte_C2 = begin_byte_C1 + (total_C1_entries + 1) * (utilities::byte_align(k) / 8);
        uint64_t total_C2_entries = ceil(total_C1_entries / static_cast<double>(kCheckpoint2Interval));
        uint64_t begin_byte_C3 = begin_byte_C2 + (total_C2_entries + 1) * (utilities::byte_align(k) / 8);

        uint32_t size_C3 = calculate_c3_size(k);
        ...
        uint8_t *right_entry_buf = new uint8_t[right_entry_size_bytes];
        uint8_t *C1_entry_buf = new uint8_t[utilities::byte_align(k) / 8];
        ...
        for (uint64_t f7_position = 0; f7_position < res.final_entries_written; f7_position++) {
            plot_file_reader.read(reinterpret_cast<char *>(right_entry_buf), right_entry_size_bytes);
            uint64_t entry_y = utilities::slice_int64_from_bytes(right_entry_buf, right_entry_size_bytes, 0, k);
            uint64_t entry_new_pos =
                utilities::slice_int64_from_bytes(right_entry_buf, right_entry_size_bytes, k, pos_size);

            bits entry_y_bits = bits(entry_y, k);

            if (f7_position % kEntriesPerPark == 0 && f7_position > 0) {
                memset(P7_entry_buf, 0, P7_park_size);
                to_write_p7.to_bytes(P7_entry_buf);
                final_file_writer_3.write(reinterpret_cast<char *>(P7_entry_buf), P7_park_size);
                to_write_p7 = park_bits();
            }

            to_write_p7 += park_bits(entry_new_pos, k + 1);

            if (f7_position % kCheckpoint1Interval == 0) {
                entry_y_bits.to_bytes(C1_entry_buf);
                final_file_writer_1.write(reinterpret_cast<char *>(C1_entry_buf), utilities::byte_align(k) / 8);
                if (num_C1_entries > 0) {
                    final_file_writer_2.seekp(begin_byte_C3 + (num_C1_entries - 1) * size_C3);
                    park_bits to_write = encoding::ans_encode_deltas(deltas_to_write, kC3R);

                    // We need to be careful because deltas are variable sized, and they need to fit
                    uint64_t num_bytes = (utilities::byte_align(to_write.size()) / 8) + 2;
                    assert(size_C3 * 8 > num_bytes);

                    // write the size, and then the data
                    bits(to_write.size() / 8, 16).to_bytes(C3_entry_buf);
                    to_write.to_bytes(C3_entry_buf + 2);
        ...
        bits(res.final_table_begin_pointers[8], 8 * 8).to_bytes(table_pointer_bytes);
        final_file_writer_1.write(reinterpret_cast<char *>(table_pointer_bytes), 8);
        bits(res.final_table_begin_pointers[9], 8 * 8).to_bytes(table_pointer_bytes);
        final_file_writer_1.write(reinterpret_cast<char *>(table_pointer_bytes), 8);
        bits(res.final_table_begin_pointers[10], 8 * 8).to_bytes(table_pointer_bytes);
        final_file_writer_1.write(reinterpret_cast<char *>(table_pointer_bytes), 8);
