...
class sort_on_disk_utils {
public:
    /*
     * Given an array of bytes, extracts an unsigned 64 bit integer from the given
     * index, to the given index.
     */
    inline static uint64_t extract_num(uint8_t *bytes, uint32_t len_bytes, uint32_t begin_bits, uint32_t take_bits) {
     ...
     */
    inline static int mem_cmp_bits(uint8_t *left_arr, uint8_t *right_arr, uint32_t len, uint32_t bits_begin) {
    ...
    // The number of memory entries required to do the custom sort_in_memory algorithm, given the total number of
    // entries to be sorted.
    inline static uint64_t round_size(uint64_t size) {
        size *= 2;
        uint64_t result = 1;
        while (result < size)
            result *= 2;
    ...
    inline static bool is_position_empty(uint8_t *memory, uint32_t entry_len) {
...
public:
    ...
    virtual std::iostream *read_handle(uint64_t begin) = 0;
    virtual std::iostream *write_handle(uint64_t begin) = 0;
};

class file_disk : public Disk {
public:
    inline explicit file_disk(const std::string& filename) {
        initialize(filename);
    }
     ...
     */
    inline std::iostream *read_handle(uint64_t begin) override {
        f_.seekg(begin);
        return &f_;
    }

    inline std::iostream *write_handle(uint64_t begin) override {
        f_.seekp(begin);
        return &f_;
    }
...
class bucket_store {
public:
    inline bucket_store(uint8_t *mem, uint64_t mem_len, uint32_t entry_len, uint32_t bits_begin, uint32_t bucket_log,
    ...
    inline void set_segment_id(uint64_t i, uint64_t v) {
        utilities::int_to_four_bytes(mem_ + i * seg_size_, v);
    }

    inline uint64_t get_segment_id(uint64_t i) {
        return utilities::four_bytes_to_int(mem_ + i * seg_size_);
    }

    // Get the first empty position from the head segment of bucket b.
    inline uint64_t get_entry_pos(uint64_t b) {
    ...
    inline void audit() {
        ...
        while (pos != length_) {
            ++count;
            pos = get_segment_id(pos);
        }
        for (uint64_t pos2 : bucket_head_ids_) {
            while (pos2 != length_) {
                ++count;
                pos2 = get_segment_id(pos2);
            }
        }
        assert(count == length_);
    }

    inline uint64_t num_free() {
        uint64_t used = get_segment_id(first_empty_seg_id_);
    ...
    inline void store(uint8_t *new_val, uint64_t new_val_len) {
        ...
        uint64_t b = sort_on_disk_utils::extract_num(new_val, new_val_len, bits_begin_, bucket_log_);
        bucket_sizes_[b] += 1;
        ...
        if (bucket_head_ids_[b] == length_ || bucket_head_counts_[b] == entries_per_seg_) {
            ...
            // form a linked list).
            first_empty_seg_id_ = get_segment_id(first_empty_seg_id_);
    ...
    inline std::vector<uint64_t> buckets_by_size() {
    ...
    // Similar to how 'bits' class works, appends an entry to the entries list, such as all entries are stored into
    // 128-bit blocks. bits class was avoided since it consumes more time than a uint128_t array.
    void add_bucket_entry(uint8_t *big_endian_bytes, uint64_t num_bytes, uint16_t size_bits, uint128_t *entries,
                          uint64_t &cnt) {
    ...
    // Extracts 'number_of_entries' from bucket b and empties memory of those from bucket_store.
    inline uint128_t *bucket_handle(uint64_t b, uint64_t number_of_entries, uint64_t &final_size) {
        ...
        while (bucket_head_ids_[b] != length_) {
            ...
            uint64_t next_full_seg_id = get_segment_id(bucket_head_ids_[b]);
            // The processed segment becomes now an empty segment.
            set_segment_id(bucket_head_ids_[b], first_empty_seg_id_);
            ...
            if (start_pos != end_pos) {
                bucket_sizes_[b] -= 1;
                add_bucket_entry(mem_ + end_pos + L, L, L * 8, entries, cnt);
                ++cnt_entries;
                if (cnt_entries == number_of_entries) {
                    final_size = cnt;
                    return entries;
...
class sorting {
public:
    static void entry_to_bytes(const uint128_t *entries, uint32_t start_pos, uint32_t end_pos, uint8_t last_size,
                               uint8_t *buffer) {
        uint8_t shift = utilities::byte_align(last_size) - last_size;
    ...
    inline static void sort_on_disk(Disk &disk, uint64_t disk_begin, uint64_t spare_begin, uint32_t entry_len,
                                    uint32_t bits_begin, std::vector<uint64_t> bucket_sizes, uint8_t *mem,
                                    uint64_t mem_len, int quicksort = 0) {
        ...
        // How much an entry occupies in memory, without the common prefix, in sort_in_memory algorithm.
        ...
        // fits in the memory (sort_in_memory algorithm won't always perform well).
        if (quicksort > 0 && total_size <= length) {
            disk.read(disk_begin, mem, total_size * entry_len);
            quick_sort(mem, entry_len, total_size, bits_begin);
            disk.write(disk_begin, mem, total_size * entry_len);
            return;
        }
        // Do sort_in_memory algorithm if it fits in the memory
        // (number of entries required * entry_len_memory) <= total memory available
        if (quicksort == 0 && sort_on_disk_utils::round_size(total_size) * entry_len_memory <= mem_len) {
            sort_in_memory(disk, disk_begin, mem, entry_len, total_size, bits_begin);
            return;
        }
        ...
        uint32_t bucket_log = utilities::get_size_bits(N_buckets) - 1;
        ...
        for (uint32_t i = 0; i < bucket_sizes.size(); i++) {
            ...
            while (to_consume > 0) {
                ...
                disk.read(disk_begin + (bucket_begins[i] + consumed_per_bucket[i]) * entry_len, mem,
                          next_amount * entry_len);
                disk.write(spare_begin + spare_written * entry_len, mem, next_amount * entry_len);
        ...
        bucket_store bstore = bucket_store(mem, mem_len, entry_len, bits_begin, bucket_log, 100);
        std::iostream *handle = disk.read_handle(spare_begin);

        uint8_t *buf = new uint8_t[entry_len];

        // Populate bucket_store from spare.
        while (!bstore.is_full() && spare_consumed < spare_written) {
        ...
        while (!bstore.is_empty()) {
            // write from bucket_store the heaviest buckets first (so it empties faster)
            for (uint64_t b : bstore.buckets_by_size()) {
                if (written_per_bucket[b] >= consumed_per_bucket[b]) {
                    continue;
                }
                // write the content of the bucket in the right spot (beginning of the bucket + number of entries
                // already written in that bucket).
                handle = disk.write_handle(disk_begin + (bucket_begins[b] + written_per_bucket[b]) * entry_len);
                uint64_t size;
                // Don't extract from the bucket more entries than the difference between read and written entries (this
                // avoids overwritting entries that were not read yet).
                uint128_t *bucket_handle =
                    bstore.bucket_handle(b, consumed_per_bucket[b] - written_per_bucket[b], size);
                ...
                for (uint64_t i = 0; i < size; i += entry_size) {
                    entry_to_bytes(bucket_handle, i, i + entry_size, last_size, buf);
                    handle->write(reinterpret_cast<char *>(buf), entry_len);
                    written_per_bucket[b] += 1;
                    subbucket_sizes[b][sort_on_disk_utils::extract_num(buf, entry_len, bits_begin + bucket_log,
                                                                       bucket_log)] += 1;
                }
                delete[] bucket_handle;
            }

            // Advance the read handle into buckets and move read entries to bucket_store. We read first from buckets
            // with the smallest difference between read and write handles. The goal is to increase the smaller
            // differences. The bigger the difference is, the better, as in the next step, we'll be able to extract more
            // from the bucket_store.
            std::vector<uint64_t> idx(bucket_sizes.size());
            iota(idx.begin(), idx.end(), 0);
            sort(idx.begin(), idx.end(), [&consumed_per_bucket, &written_per_bucket](uint64_t i1, uint64_t i2) {
            ...
            for (uint64_t i : idx) {
                ...
                std::iostream *handle2 =
                    disk.read_handle(disk_begin + (bucket_begins[i] + consumed_per_bucket[i]) * entry_len);
                while (!bstore.is_full() && consumed_per_bucket[i] < bucket_sizes[i]) {
            ...
            // If bucket_store still isn't full and we've read all entries from buckets, start populating from the spare
            // space.
            if (!broke) {
                std::iostream *handle3 = disk.read_handle(spare_begin + spare_consumed * entry_len);
                while (!bstore.is_full() && spare_consumed < spare_written) {
        ...
        for (std::size_t i = 0; i < bucket_sizes.size(); i++) {
            ...
            sort_on_disk(disk, disk_begin + bucket_begins[i] * entry_len, spare_begin, entry_len,
                         bits_begin + bucket_log, subbucket_sizes[i], mem, mem_len, new_quicksort);
        }
    }

    inline static void sort_in_memory(Disk &disk, uint64_t disk_begin, uint8_t *memory, uint32_t entry_len,
                                      uint64_t num_entries, uint32_t bits_begin) {
        uint32_t entry_len_memory = entry_len - bits_begin / 8;
        uint64_t memory_len = sort_on_disk_utils::round_size(num_entries) * entry_len_memory;
        ...
        std::iostream *read_handle = disk.read_handle(disk_begin);
        uint64_t buf_size = 0;
        uint64_t buf_ptr = 0;
        uint64_t swaps = 0;
        for (uint64_t i = 0; i < num_entries; i++) {
            ...
            uint64_t pos = sort_on_disk_utils::extract_num(buffer + buf_ptr, entry_len, bits_begin, bucket_length) *
                           entry_len_memory;
            // As long as position is occupied by a previous entry...
            while (!sort_on_disk_utils::is_position_empty(memory + pos, entry_len_memory) && pos < memory_len) {
                // ...store there the minimum between the two and continue to push the higher one.
                if (sort_on_disk_utils::mem_cmp_bits(memory + pos, buffer + buf_ptr + bits_begin / 8, entry_len_memory,
                                                     0) > 0) {
        ...
        std::iostream *write_handle = disk.write_handle(disk_begin);
        ...
        for (uint64_t pos = 0; entries_written < num_entries && pos < memory_len; pos += entry_len_memory) {
            if (!sort_on_disk_utils::is_position_empty(memory + pos, entry_len_memory)) {
    ...
    inline static void quick_sort(uint8_t *memory, uint32_t entry_len, uint64_t num_entries, uint32_t bits_begin) {
        uint64_t memory_len = (uint64_t)entry_len * num_entries;
        uint8_t *pivot_space = new uint8_t[entry_len];
        quick_sort_inner(memory, memory_len, entry_len, bits_begin, 0, num_entries, pivot_space);
        delete[] pivot_space;
    }

    inline static void check_sort_on_disk(Disk &disk, uint64_t disk_begin, uint64_t spare_begin, uint32_t entry_len,
                                          uint32_t bits_begin, const std::vector<uint64_t> &bucket_sizes, uint8_t *mem,
                                          uint64_t mem_len, bool quicksort = false) {
        uint64_t length = floor(mem_len / entry_len);
        uint64_t total_size = 0;
        for (auto &n : bucket_sizes)
            total_size += n;

        std::cout << "check_sort_on_disk entry_len: " << entry_len << " length: " << length
        ...
        for (uint64_t chunkindex = 0; chunkindex < (total_size + length - 1) / length; chunkindex++) {
            ...
            while (((chunkindex * length + i) < total_size) && (i < length)) {
                if ((chunkindex * length + i) % 1000000 == 0)
                    std::cout << (chunkindex * length + i) << ": " << utilities::hex_str(mem + i * entry_len, entry_len)
                              << std::endl;
                if (sort_on_disk_utils::mem_cmp_bits(mem + (i - 1) * entry_len, mem + i * entry_len, entry_len, 0) >=
                    0) {
        ...
        std::cout << "check_sort_on_disk OK" << std::endl;
    }

private:
    inline static void quick_sort_inner(uint8_t *memory, uint64_t memory_len, uint32_t L, uint32_t bits_begin,
        ...
        if (end - begin <= 5) {
            for (uint64_t i = begin + 1; i < end; i++) {
                ...
                while (j > begin &&
                       sort_on_disk_utils::mem_cmp_bits(memory + (j - 1) * L, pivot_space, L, bits_begin) > 0) {
        ...
        while (lo < hi) {
            if (left_side) {
                if (sort_on_disk_utils::mem_cmp_bits(memory + lo * L, pivot_space, L, bits_begin) < 0) {
                    ++lo;
                } else {
                ...
                if (sort_on_disk_utils::mem_cmp_bits(memory + hi * L, pivot_space, L, bits_begin) > 0) {
                    --hi;
                } else {
        ...
        if (lo - begin <= end - lo) {
            quick_sort_inner(memory, memory_len, L, bits_begin, begin, lo, pivot_space);
            quick_sort_inner(memory, memory_len, L, bits_begin, lo + 1, end, pivot_space);
        } else {
            quick_sort_inner(memory, memory_len, L, bits_begin, lo + 1, end, pivot_space);
            quick_sort_inner(memory, memory_len, L, bits_begin, begin, lo, pivot_space);
        }
