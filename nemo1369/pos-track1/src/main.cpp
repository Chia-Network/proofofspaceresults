...
std::vector<unsigned char> int_to_bytes(uint32_t paramInt, uint32_t numBytes) {
    ...
    for (std::size_t i = 0; paramInt > 0; i++) {
...
int main(int argc, char *argv[]) {
    ...
    boost::asio::detail::thread_group tg;
    tg.create_threads(boost::bind(&boost::asio::io_service::run, &io_service), std::thread::hardware_concurrency());
