 ...
 * NOTE: calculate_bucket only accepts a bits_generic<small_vector>, so in order to use that, you have to firstly
 * convert your bits_generic<T> object into a bits_generic<small_vector>.
 */
...
public:
    ...
    bits_generic<T>(uint8_t *big_endian_bytes, uint32_t num_bytes, uint32_t size_bits) {
        this->last_size_ = 0;
