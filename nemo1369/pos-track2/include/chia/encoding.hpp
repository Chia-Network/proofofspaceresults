...
#include "../../libs/FiniteStateEntropy/lib/hist.h"
#include "../../libs/FiniteStateEntropy/lib/fse.h"
...
public:
    ...
    static uint128_t square_to_line_point(uint64_t x, uint64_t y) {
    ...
    static std::pair<uint64_t, uint64_t> line_point_to_square(uint128_t index) {
        ...
        return {x, index - (((uint128_t)x * (x - 1)) / 2)};
    }

    static std::vector<short> create_normalized_count(double R) {
        ...
        auto cmp = [&dpdf, &ans](int i, int j) {
            return dpdf[i] * (log2(ans[i] + 1) - log2(ans[i])) < dpdf[j] * (log2(ans[j] + 1) - log2(ans[j]));
        };
    ...
    static park_bits ans_encode_deltas(const std::vector<unsigned char> &deltas, double R) {
        if (CT_MEMO.find(R) == CT_MEMO.end()) {
            std::vector<short> nCount = encoding::create_normalized_count(R);
        ...
        uint64_t num_bytes = FSE_compress_usingCTable(out, deltas.size() * 8, deltas.data(), deltas.size(), CT_MEMO[R]);

        park_bits res = park_bits(reinterpret_cast<uint8_t *>(out), num_bytes, num_bytes * 8);
        free(out);
        return res;
    }

    template<typename X>
    static std::vector<uint8_t> ans_decode_deltas(X bits, int numDeltas, double R) {
        ...
        void *inp = std::malloc(numDeltas * 8);
        std::memset(inp, 0x00, numDeltas * 8);
        int inpsize = utilities::byte_align(bits.size()) / 8;
        void *out = malloc(numDeltas);
        std::memset(out, 0x00, numDeltas);
        bits.to_bytes(reinterpret_cast<uint8_t *>(inp));
        ...
        free(inp);
        free(out);
        for (unsigned char delta : deltas) {
            if (delta == 0xff) {
