...
public:
    ...
    void print_elapsed(const std::string &name) {
        ...
        auto wall_clock_ms =
            std::chrono::duration_cast<std::chrono::milliseconds>(end - this->wall_clock_time_start_).count();
...
public:
    ...
    static uint32_t byte_align(uint32_t num_bits) {
    ...
    static void write_zeroes_heap(std::ofstream &file, uint32_t num_bytes) {
    ...
    static void write_zeroes_stack(std::ofstream &file, uint32_t num_bytes) {
     ...
     */
    static void int_to_four_bytes(uint8_t *result, const uint32_t input) {
     ...
     */
    static uint32_t four_bytes_to_int(const uint8_t *bytes) {
        uint32_t sum = 0;
        for (size_t i = 0; i < 4; i++) {
     ...
     */
    static uint64_t eight_bytes_to_int(const uint8_t *bytes) {
        uint64_t sum = 0;
     ...
     */
    static uint8_t get_size_bits(uint128_t value) {
        uint8_t count = 0;
        while (value) {
            count++;
            value >>= 1;
    ...
    inline static uint64_t slice_int64_from_bytes(const uint8_t *bytes, const uint32_t bytes_len,
                                                  const uint32_t start_bit, const uint32_t num_bits) {
        // assert(utilities::byte_align(start_bit + num_bits) <= bytes_len * 8);
        assert(num_bits <= 64);
    ...
    inline static uint128_t slice_int128_from_bytes(const uint8_t *bytes, const uint32_t bytes_len,
                                                    const uint32_t start_bit, const uint32_t num_bits) {
        assert(utilities::byte_align(start_bit + num_bits) <= bytes_len * 8);
        uint128_t sum = 0;
        uint32_t taken_bits = 0;
    ...
    static uint64_t find_islands(const std::vector<std::pair<uint64_t, uint64_t>> &edges) {
