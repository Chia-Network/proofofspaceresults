...
public:
     ...
     */
    inline static uint64_t extract_num(const uint8_t *bytes, uint32_t len_bytes, uint32_t begin_bits,
                                       uint32_t take_bits) {
    ...
    inline static bool is_position_empty(const uint8_t *memory, uint32_t entry_len) {
...
public:
     ...
     */
    inline std::iostream *read_handle(uint64_t begin) {
        f_.seekg(begin);
        return &f_;
    }

    inline std::iostream *write_handle(uint64_t begin) {
        f_.seekp(begin);
        return &f_;
    }
...
public:
    ...
    void add_bucket_entry(const uint8_t *big_endian_bytes, uint64_t num_bytes, uint16_t size_bits, uint128_t *entries,
                          uint64_t &cnt) {
