...
public:
    ...
    std::vector<uint64_t> get_p_7_positions(uint64_t curr_f7, uint64_t f7, uint64_t curr_p7_pos, uint8_t *bit_mask,
    ...
    std::vector<uint64_t> get_p7_entries(uint8_t *challenge) {
        ...
        if (double_entry) {
            ...
            p7_positions = get_p_7_positions(curr_f7, f7, curr_p7_pos, bit_mask, encoded_size, c1_index);
            ...
            curr_f7 = next_f7;
            auto second_positions = get_p_7_positions(next_f7, f7, curr_p7_pos, bit_mask, encoded_size, c1_index);
    ...
    large_bits get_full_proof(uint8_t *challenge, uint32_t index) {
        ...
        std::vector<large_bits> xs_sorted = reorder_proof(xs);
        large_bits full_proof;
        ...
        for (uint8_t table_index = 2; table_index < 8; table_index++) {
            ...
            for (uint8_t i = 0; i < results.size(); i += 2) {
                ...
                if (std::get<0>(results[i]).get_value() < std::get<0>(results[i + 1]).get_value()) {
                    new_output = f.calculate_bucket(std::get<0>(results[i]), std::get<0>(results[i + 1]),
                    ...
                    new_output = f.calculate_bucket(std::get<0>(results[i + 1]), std::get<0>(results[i]),
                ...
                assert(std::get<0>(new_output).size() != 0);
