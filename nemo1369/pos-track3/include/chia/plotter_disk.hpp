...
public:
    ...
    void create_plot_disk(const std::string &filename, uint8_t k, uint8_t *memo, uint32_t memo_len, uint8_t *id,
        ...
        Timer hellman_timer;
        build_extra_storage(k, id, plot_filename, extra_metadata);
        hellman_timer.print_elapsed("Time for phrase 0 = ");
        ...
        Timer p3;
        phase3_results res = compress_tables(k, results, filename, plot_filename, id, memo, memo_len, extra_metadata);
        ...
        extra_storage_timer.print_elapsed("Time for extra storage = ");
        ...
        switch (table_index) {
            ...
            case 6:
                if (phase_1_size)
                    ...
                    return utilities::byte_align(max(static_cast<uint32_t>(k + 1 + (k + 1) + kOffsetSize),
    ...
    static uint32_t calculate_max_deltas_size(uint8_t k, uint8_t table_index) {
        ...
        if (table_index == 2) {
            return utilities::byte_align(std::floor((kEntriesPerPark - 1) * (kMaxAverageDelta + 1))) / 8;
        }
...
private:
    ...
    std::vector<uint64_t> write_plot_file(const std::string &plot_filename, uint8_t k, uint8_t *id, uint8_t *memo,
        ...
        for (uint64_t lp = 0; lp <= (((uint64_t)1) << (k - kBatchSizes)); lp++) {
            ...
            for (auto kv : f1.calculate_buckets(bits(x, k), 2 << (kBatchSizes - 1))) {
                // TODO(mariano): fix inefficient memory alloc here
                (std::get<0>(kv) + std::get<1>(kv)).to_bytes(buf);
        ...
        for (uint8_t table_index = 1; table_index < 7; table_index++) {
            ...
            std::vector<plot_entry> bucket_L;
            std::vector<plot_entry> bucket_R;
            ...
            while (!end_of_table) {
                ...
                if (y_bucket == bucket) {
                    ...
                    if (!bucket_L.empty() && !bucket_R.empty()) {
                        ...
                        std::vector<std::pair<uint16_t, uint16_t>> match_indexes = f.find_matches(bucket_L, bucket_R);
                        for (auto &indeces : match_indexes) {
                            ...
                            if (metadata_size <= 128) {
                                f_output = f.calculate_bucket(bits(L_entry.y, k + kExtraBits),
                            ...
                            new_entry.append_value(R_entry.pos - L_entry.pos, kOffsetSize);
                            ...
                            new_entry.append_value(0, right_entry_size_bytes * 8 - new_entry.size());
                            new_entry.to_bytes(right_buf);
    ...
    void write_park_to_file(std::ofstream &writer, uint64_t table_start, uint64_t park_index, uint32_t park_size_bytes,
        ...
        if (encoded_size + 2 > calculate_max_deltas_size(k, table_index))
            std::cout << encoded_size + 2 << " " << calculate_max_deltas_size(k, table_index) << "\n";
        assert(encoded_size + 2 < calculate_max_deltas_size(k, table_index));
    ...
    phase3_results compress_tables(uint8_t k, std::vector<uint64_t> plot_table_begin_pointers,
                                   const std::string &filename, const std::string &plot_filename, uint8_t *id,
                                   uint8_t *memo, uint32_t memo_len, std::vector<uint64_t> &extra_metadata) {
        ...
        for (uint8_t table_index = 1; table_index < 7; table_index++) {
                ...
                if (end_of_right_table || current_pos <= greatest_pos) {
                    ...
                    if (table_index == 1) {
                        ...
                        left_new_pos[current_pos % kCachedPositionsSize] =
                            utilities::slice_int64_from_bytes(left_entry_disk_buf, left_entry_size_bytes, k + 1, k);
                    } else {
            ...
            for (uint64_t index = 0; index < total_r_entries; index++) {
                ...
                if (table_index > 1) {
                    ...
                    auto x1x2 = encoding::line_point_to_square(line_point);
                    bits y1 = f1.calculate_f(bits(x1x2.first, k));
                    bits y2 = f1.calculate_f(bits(x1x2.second, k));
                    if (y1 < y2)
            ...
            if (table_index > 1) {
                ...
                final_entries_written = 0;
                uint8_t metadata_len = utilities::byte_align(k) / 8;
                uint8_t buf[metadata_len];
                bits num_entries(extra_metadata.size(), k);
                num_entries.to_bytes(buf);
                ...
                std::cout << "\tWrote " << final_entries_written << " entries" << std::endl;
                final_table_begin_pointers[table_index + 1] =
                    final_table_begin_pointers[table_index] + (final_entries_written + 2) * metadata_len;
            }
    ...
    void build_extra_storage(int k, uint8_t *id, const string &filename, std::vector<uint64_t> &extra_metadata) {
        attacker_type attacker(pow(2, ((double)k * 2 / 3)), pow(2, ((double)k / 3)), (1LL << k), 5, id);
        ...
        attacker.build_disk_extra_storage(filename, extra_metadata);
