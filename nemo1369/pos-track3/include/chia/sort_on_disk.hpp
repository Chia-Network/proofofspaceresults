...
public:
    ...
    // The number of memory entries required to do the custom SortInMemory algorithm, given the total number of entries
    // to be sorted.
...
class BucketStore {
public:
    inline BucketStore(uint8_t* mem, uint64_t mem_len, uint32_t entry_len, uint32_t bits_begin, uint32_t bucket_log,
    ...
    void AddBucketEntry(uint8_t* big_endian_bytes, uint64_t num_bytes, uint16_t size_bits, uint128_t* entries,
                        uint64_t& cnt) {
...
class sorting {
public:
    static void EntryToBytes(uint128_t* entries, uint32_t start_pos, uint32_t end_pos, uint8_t last_size,
                             uint8_t buffer[]) {
    ...
    inline static void sort_on_disk(Disk& disk, uint64_t disk_begin, uint64_t spare_begin, uint32_t entry_len,
        ...
        if (quicksort == 0 && sort_on_disk_utils::RoundSize(total_size) * entry_len_memory <= mem_len) {
        ...
        while (!bstore.IsEmpty()) {
            ...
            for (uint64_t i : idx) {
                ...
                std::iostream* handle2 =
                    disk.ReadHandle(disk_begin + (bucket_begins[i] + consumed_per_bucket[i]) * entry_len);
            ...
            // If BucketStore still isn't full and we've read all entries from buckets, start populating from the spare
            // space.
            if (!broke) {
                std::iostream* handle3 = disk.ReadHandle(spare_begin + spare_consumed * entry_len);
                while (!bstore.IsFull() && spare_consumed < spare_written) {
    ...
    inline static void SortInMemory(Disk& disk, uint64_t disk_begin, uint8_t* memory, uint32_t entry_len,
        ...
        uint32_t entry_len_memory = entry_len - bits_begin / 8;
        uint64_t memory_len = sort_on_disk_utils::RoundSize(num_entries) * entry_len_memory;
        ...
        for (uint64_t i = 0; i < num_entries; i++) {
            ...
            uint64_t pos = sort_on_disk_utils::extract_num(buffer + buf_ptr, entry_len, bits_begin, bucket_length) * entry_len_memory;
            // As long as position is occupied by a previous entry...
            while (!sort_on_disk_utils::IsPositionEmpty(memory + pos, entry_len_memory) && pos < memory_len) {
                // ...store there the minimum between the two and continue to push the higher one.
                if (sort_on_disk_utils::MemCmpBits(memory + pos, buffer + buf_ptr + bits_begin / 8, entry_len_memory, 0) >
                    0) {
        ...
        for (uint64_t pos = 0; entries_written < num_entries && pos < memory_len; pos += entry_len_memory) {
            if (!sort_on_disk_utils::IsPositionEmpty(memory + pos, entry_len_memory)) {
    ...
    inline static void QuickSort(uint8_t* memory, uint32_t entry_len, uint64_t num_entries, uint32_t bits_begin) {
    ...
    inline static void CheckSortOnDisk(Disk& disk, uint64_t disk_begin, uint64_t spare_begin, uint32_t entry_len,
        ...
        std::cout << "CheckSortOnDisk entry_len: " << entry_len << " length: " << length << " total size: " << total_size
             << std::endl;
        for (uint64_t chunkindex = 0; chunkindex < (total_size + length - 1) / length; chunkindex++) {
            ...
            while (((chunkindex * length + i) < total_size) && (i < length)) {
                if ((chunkindex * length + i) % 1000000 == 0)
                    std::cout << (chunkindex * length + i) << ": " << utilities::hex_str(mem + i * entry_len, entry_len) << std::endl;
                if (sort_on_disk_utils::MemCmpBits(mem + (i - 1) * entry_len, mem + i * entry_len, entry_len, 0) >= 0) {
                    std::cout << "Bad sort!" << std::endl;
                }
                i++;
            }
        }
        std::cout << "CheckSortOnDisk OK" << std::endl;
    }

private:
    inline static void QuickSortInner(uint8_t* memory, uint64_t memory_len, uint32_t L, uint32_t bits_begin,
        ...
        if (end - begin <= 5) {
            for (uint64_t i = begin + 1; i < end; i++) {
                ...
                while (j > begin &&
                       sort_on_disk_utils::MemCmpBits(memory + (j - 1) * L, pivot_space, L, bits_begin) > 0) {
        ...
        while (lo < hi) {
            if (left_side) {
                if (sort_on_disk_utils::MemCmpBits(memory + lo * L, pivot_space, L, bits_begin) < 0) {
                    ++lo;
                } else {
                ...
                if (sort_on_disk_utils::MemCmpBits(memory + hi * L, pivot_space, L, bits_begin) > 0) {
                    --hi;
                } else {
