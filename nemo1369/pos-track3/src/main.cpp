...
void hex_to_bytes(const string &hex, uint8_t *result) {
...
int main(int argc, char *argv[]) {
    ...
    if (vm.count("help") || argc < 2) {
        ...
        std::cout << "Generating plot for k=" << static_cast<int>(k) << " filename=" << filename << " id=" << id << endl
                  << std::endl;
        if (id.size() != 64) {
        ...
        std::cout << "Proving using filename=" << filename << " challenge=" << challenge << endl << std::endl;
        ...
        for (uint32_t i = 0; i < qualities.size(); i++) {
            ...
            large_bits proof = prover.get_full_proof(challenge_bytes, i);
        ...
        if (quality.size() == 2 * k) {
            std::cout << "Proof verification suceeded. Quality: " << quality << std::endl;
        } else {
        ...
        for (uint32_t num = 0; num < iterations; num++) {
            ...
            for (uint32_t i = 0; i < qualities.size(); i++) {
                ...
                proof.to_bytes(proof_data);
                std::cout << "i: " << num << " Proof: 0x" << utilities::hex_str(proof_data, k * 8) << std::endl;
                large_bits quality = ver.validate_proof(id_bytes, k, hash.data(), proof_data, k * 8);
                if (quality.size() == 2 * k) {
                    std::cout << "Proof verification succeeded. k = " << static_cast<int>(k) << " Quality: " << quality
                              << std::endl;
                    success++;
                } else {
                    std::cout << "Proof verification failed." << std::endl;
