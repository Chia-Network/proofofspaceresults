...
// on error return -1
int FPC_decompress_block(void *output, int out_size, const void *input,
                         int in_size, int sym_num);
...
size_t FPC_decompress(void *output, size_t max_output, void *input,
                      size_t inlen);

#define FPC_MAX_OUTPUT(S, B)                                                   \
  ((S) + 256 + (B == 0 ? (((S) / (32 * 1024)) + 1) * 4 : ((S) / (B) + 1) * 4))
