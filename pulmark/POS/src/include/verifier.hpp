...
class Verifier {
public:
  // Gets the quality string from a proof in proof ordering. The quality string
  // is two adjacent values, determined by the quality index (1-32), and the
  // proof in plot ordering.
  LargeBits GetQualityString(uint8_t k, LargeBits proof,
  ...
  // Validates a proof of space, and returns the quality string if the proof is
  // valid for the given k and challenge. If the proof is invalid, it returns an
  // empty LargeBits().
  LargeBits ValidateProof(uint8_t *id, uint8_t k, uint8_t *challenge,
                          uint8_t *proof_bytes, uint16_t proof_size) {
    LargeBits proof_bits = LargeBits(proof_bytes, proof_size, proof_size * 8);
    if (k * 64 != proof_bits.GetSize()) {
      m_assert(0, "validate proof: wrong proof size");
      return LargeBits();
    }
    ...
    // Calculates f1 for each of the given xs. Note that the proof is in proof
    // order.
    ...
    // Calculates fx for each table from 2..7, making sure everything matches on
    // the way.
    for (uint8_t depth = 2; depth < 8; depth++) {
      ...
      for (uint8_t i = 0; i < (1 << (8 - depth)); i += 2) {
        ...
        if (f.FindMatches(bucket_L, bucket_R).size() != 1) {
          m_assert(0, "validate proof: no matches found");
          return LargeBits();
        }
        std::pair<Bits, Bits> results = f.CalculateBucket(
            ys[i], /* ys[i + 1], */ metadata[i], metadata[i + 1]);
      ...
      for (uint8_t i = 0; i < new_ys.size(); i++) {
        if (new_ys[i].GetSize() <= 0) {
          m_assert(0, "validate proof: new_ys[i] size <= 0");
          return LargeBits();
        }
      }
    ...
    if (challenge_bits.Slice(0, k) == ys[0].Slice(0, k)) {
      ...
      m_assert(0, "validate proof: output not equal with challenge");
      return LargeBits();
    }
  }
