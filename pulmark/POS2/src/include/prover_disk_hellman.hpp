...
#ifndef PROVER_DISK_HELLMAN_HPP
#define PROVER_DISK_HELLMAN_HPP
...
public:
  ...
  explicit DiskProver(std::string filename)
    ...
    disk_file.read(reinterpret_cast<char *>(format_description_read),
    ...
    for (uint i = 0;
         i <
         floor((table_begin_pointers[10] - table_begin_pointers[9]) / c2_size) -
             1;
         i++) {
      disk_file.read(reinterpret_cast<char *>(c2_buf), c2_size);
      this->C2.push_back(
          Bits(c2_buf, c2_size, c2_size * 8).Slice(0, k).GetValue());
    }

    attacker = new Attacker(pow(2, ((double)k * 2 / 3)),
                            pow(2, ((double)k / 3)), (1LL << k), 5, id);
  ...
  void GetMemo(uint8_t *buffer) { std::memcpy(buffer, memo, this->memo_size); }

  void GetId(uint8_t *buffer) { std::memcpy(buffer, id, kIdLen); }

  uint8_t GetSize() { return k; }
  ...
  uint128_t ReadLinePoint(uint8_t table_index, uint64_t position) {
    ...
    uint32_t park_size_bits =
        DiskPlotter::CalculateParkSize(k, table_index) * 8;
    disk_file.seekg(table_begin_pointers[table_index] +
    ...
    disk_file.read(reinterpret_cast<char *>(line_point_bin),
                   line_point_size_bits / 8);
    uint128_t line_point =
        Bits(line_point_bin, line_point_size_bits / 8, line_point_size_bits)
            .Slice(0, k * 2)
            .GetValue();
    ...
    for (uint32_t i = 0; i < kEntriesPerPark - 1; i++) {
      ParkBits stubs_section =
          ParkBits(stubs_bin + (i * (k - kStubMinusBits)) / 8,
                   (Util::ByteAlign((k - kStubMinusBits)) / 8) + 1,
                   (Util::ByteAlign((k - kStubMinusBits)) / 8 + 1) * 8);
      uint8_t start_bit = (i * (k - kStubMinusBits)) % 8;
      stubs.push_back(
          stubs_section.Slice(start_bit, start_bit + (k - kStubMinusBits))
              .GetValue());
    }

    // Reads EPP deltas
    uint32_t max_deltas_size_bits =
        DiskPlotter::CalculateMaxDeltasSize(k, table_index) * 8;
    ...
    disk_file.read(reinterpret_cast<char *>(&encoded_deltas_size),
                   sizeof(uint16_t));
    disk_file.read(reinterpret_cast<char *>(deltas_bin), encoded_deltas_size);
    ParkBits deltas_park =
        ParkBits(deltas_bin, encoded_deltas_size, encoded_deltas_size * 8);
    ...
    uint128_t big_delta =
        ((uint128_t)1 << (k - kStubMinusBits)) * sum_deltas + sum_stubs;
  ...
  // encoded.
  std::vector<uint64_t> GetP7Positions(uint64_t curr_f7, uint64_t f7,
                                       uint64_t curr_p7_pos, uint8_t *bit_mask,
  ...
  std::vector<std::pair<uint64_t, uint64_t>> InvertF2(uint64_t x0,
    ...
    for (auto x1 : inv1) {
      ...
      for (auto x3 : inv2) {
        ...
        l_plot_entry.y = fx.CalculateBucket(y0, y1, Bits(x0, k), Bits(x1, k))
                             .first.GetValue();
        r_plot_entry.y = fx.CalculateBucket(y2, y3, Bits(x2, k), Bits(x3, k))
                             .first.GetValue();
        std::vector<PlotEntry> bucket_L = {l_plot_entry};
        std::vector<PlotEntry> bucket_R = {r_plot_entry};
        if (l_plot_entry.y < r_plot_entry.y &&
            fx.FindMatches(bucket_L, bucket_R).size() == 1) {
          res.push_back({x1, x3});
        }
        if (l_plot_entry.y > r_plot_entry.y &&
            fx.FindMatches(bucket_R, bucket_L).size() == 1) {
          res.push_back({x1, x3});
        }
      }
    }
  ...
  std::vector<uint64_t> GetP7Entries(uint8_t *challenge) {
    ...
    disk_file.seekg(table_begin_pointers[8] +
    ...
    for (uint64_t start = 0; start < kCheckpoint1Interval; start++) {
      ...
      Bits c1_entry =
          Bits(c1_entry_bytes, Util::ByteAlign(k) / 8, Util::ByteAlign(k));
    ...
    if (double_entry) {
      ...
      disk_file.seekg(table_begin_pointers[8] +
                      c1_index * Util::ByteAlign(k) / 8);
      disk_file.read(reinterpret_cast<char *>(c1_entry_bytes),
                     Util::ByteAlign(k) / 8);
      Bits c1_entry_bits =
          Bits(c1_entry_bytes, Util::ByteAlign(k) / 8, Util::ByteAlign(k));
      next_f7 = curr_f7;
      ...
      p7_positions = GetP7Positions(curr_f7, f7, curr_p7_pos, bit_mask,
      ...
      curr_f7 = next_f7;
      auto second_positions = GetP7Positions(next_f7, f7, curr_p7_pos, bit_mask,
                                             encoded_size, c1_index);
    ...
    uint64_t p7_park_size_bytes =
        Util::ByteAlign((k + 1) * kEntriesPerPark) / 8;

    std::vector<uint64_t> p7_entries;

    // Given the p7 positions, which are all adjacent, we can read the pos6
    // values from table P7.
    ...
    ParkBits p7_park =
        ParkBits(p7_park_buf, p7_park_size_bytes, p7_park_size_bytes * 8);
    for (uint64_t i = 0;
      ...
      if (new_park_index > park_index) {
        disk_file.seekg(table_begin_pointers[7] +
                        new_park_index * p7_park_size_bytes);
        disk_file.read(reinterpret_cast<char *>(p7_park_buf),
                       p7_park_size_bytes);
        p7_park =
            ParkBits(p7_park_buf, p7_park_size_bytes, p7_park_size_bytes * 8);
      }
      ...
      uint64_t p7_int =
          p7_park.Slice(start_bit_index, start_bit_index + k + 1).GetValue();
    ...
    for (uint8_t table_index = 2; table_index < 8; table_index++) {
      ...
      for (uint8_t i = 0; i < results.size(); i += 2) {
        ...
        if (std::get<0>(results[i]).GetValue() <
          ...
          uint64_t end =
              (uint64_t)k * (i + 2) * ((uint64_t)1 << (table_index - 2));
          ...
          uint64_t start = (uint64_t)k * i * ((uint64_t)1 << (table_index - 2));
          uint64_t start2 =
              (uint64_t)k * (i + 1) * ((uint64_t)1 << (table_index - 2));
          uint64_t end =
              (uint64_t)k * (i + 2) * ((uint64_t)1 << (table_index - 2));
